...
class MainWindow(QMainWindow):
    ...
    def initMain(self):
        ...
        # border-image: url(:/icons/ui_imgs/icons/camera.png)
    ...
    def open_img(self):
        if self.cap:
            ...
            self.ui.CaplineEdit.setText('Camera turned off')
            self.cap = None
        ...
        #     now_img = tools.drawRectBox(now_img, loacation, Config.EN_names[type_id], self.fontC, color)
        ...
        #     choose_list.append(Config.EN_names[each])
        ...
        if target_nums >= 1:
            self.ui.type_lb.setText(Config.EN_names[self.cls_list[0]])
    ...
    def combox_change(self):
        ...
        if com_text == 'All':
            ...
            self.ui.type_lb.setText(Config.EN_names[self.cls_list[index]])
    ...
    def tabel_info_show(self, locations, clses, confs, path=None):
        ...
        for location, cls, conf in zip(locations, clses, confs):
            ...
            item_cls = QTableWidgetItem(str(Config.EN_names[cls]))
    ...
    def camera_show(self):
        ...
        if self.is_camera_open:
            self.ui.CaplineEdit.setText('Camera turn on')
    ...
    def save_detect_video(self):
        if self.cap is None and not self.org_path:
            QMessageBox.about(self, 'Tips', 'Nothing can be saved！')
            return

        if self.is_camera_open:
            QMessageBox.about(self, 'Tips', 'video cannot be saved!')
            return

        if self.cap:
            res = QMessageBox.information(self, 'Tips', 'Save video need long time, you sure？',QMessageBox.Yes | QMessageBox.No ,  QMessageBox.Yes)
            ...
            if os.path.isfile(self.org_path):
                ...
                QMessageBox.about(self, 'Tips', 'picture is saved!\nlocation:{}'.format(save_img_path))
            else:
                img_suffix = ['jpg', 'png', 'jpeg', 'bmp']
                ...
                QMessageBox.about(self, 'Tips', 'picture is saved!\nlocation:{}'.format(Config.save_path))
...
class btn2Thread(QThread):
    ...
    def run(self):
        ...
        print("[INFO] all frame：{}".format(total))
        cur_num = 0

        while (cap.isOpened() and self.is_running):
            cur_num += 1
            print('current frame{}，All{}'.format(cur_num, total))
